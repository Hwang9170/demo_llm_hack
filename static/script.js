...
const pageToFile = {
  ...
  create: '/static/pages/create.html',
  library: '/static/pages/library.html',
  reader: '/static/pages/reader.html',
...
async function loadAssetsForPage(page) {
  ...
  for (const url of assets.css) {
    ...
    link.rel = 'stylesheet';
    link.href = `/static/${url}?v=${ASSET_VERSION}`;
  ...
  for (const url of assets.js) {
    ...
    await new Promise((resolve, reject) => {
      ...
      s.src = `/static/${url}?v=${ASSET_VERSION}`;
      s.onload = resolve;
      s.onerror = reject;
...
async function generateBook() {
    ...
    try {
        ...
        const content = paragraphs.map((p, index) => ({
            text: p,
            illustration: images[index]?.file_path || "" // 이미지 경로 또는 빈 값
        }));
        ...
        const content = await generateBookContent(childInfo, bookTitle, bookTheme || (keywordsArr[0] || ""), pageCount);
        if (content.length === 0) {
...
// 책 내용 생성 (API 연동)
async function generateBookContent(childInfo, title, theme, pageCount) {
  try {
    const response = await apiFetch("/api/story/make", {
      method: "POST",
      body: JSON.stringify({
        childInfo,
        title,
    ...
    const storyText = String(response.story || "").trim();
    const paragraphs = storyText.split(/\n\s*\n+/).filter(Boolean);

    return paragraphs.slice(0, Math.min(pageCount, paragraphs.length)).map((text) => ({
      text,
      illustration: "📖✨",
    }));
  } catch (error) {
    console.error("generateBookContent 오류:", error.message);
...
function displayGeneratedContent(content) {
  ...
  if (!Array.isArray(content)) {
    console.error("displayGeneratedContent 오류: content가 배열이 아닙니다.", content);
    alert("동화 생성에 실패했습니다. 다시 시도해주세요.");
    return;
  }
...
// 현재 페이지 TTS 재생 함수 (전역 범위에서 정의)
window.playTTSForCurrentPage = async function () {
  try {
    if (!currentBook || currentBookPage >= currentBook.content.length) {
      alert("현재 페이지의 동화 내용을 찾을 수 없습니다.");
      return;
    }

    const currentPageContent = currentBook.content[currentBookPage]?.text || "";
    if (!currentPageContent) {
      alert("현재 페이지의 동화 내용이 비어 있습니다.");
      return;
    }
